...
const HomePage = ({filterArray}) => {
 ...
 const [selectedOption, setselectedOption] = useState(20)
  useEffect(() => {
    ...
    if (currentPage !== 1) {
    setLoading(true)
    getPhotos(currentPage, 100).then(data => setPhotos(data.slice(0, selectedOption))).then(setLoading(false))
  ...
  const handlePageClick = (data) => {
   ...
   return (
    ...
    {loading ===false? (
      ...
      nextClassName={currentPage !==1 && photos.length < selectedOption? 'buttonDisable' :'buttonNext'}
